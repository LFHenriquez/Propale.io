...
Shortcodes
----------------------------------------------*/
add_shortcode( 'slides', 'be_slides_shortcode' );

/*----------------------------------------------
Functions
----------------------------------------------*/
if ( !function_exists( 'be_slides_shortcode' ) ) {
	function be_slides_shortcode( $atts ) {
		...
		$atts = shortcode_atts( array(
			...
			'time'					=> '',
			'title'					=> '',
			'check_if_user_has_tags'=> true,
			'ignore-wrapper'		=> true,
			'wrapper'				=> 'ul',
			'wrapper_class'			=> 'slides-listing',
			'wrapper_id'			=> false,
		), $atts, 'slides' );
		...
		$check_if_user_has_tags = filter_var( $atts['check_if_user_has_tags'], FILTER_VALIDATE_BOOLEAN );
		...
		$ignore_wrapper       = filter_var( $atts['ignore-wrapper'], FILTER_VALIDATE_BOOLEAN );
		 ...
		 */
		$listing = new WP_Query( apply_filters( 'slides_shortcode_args', $args, $original_atts ) );
		$user_id = get_current_user_id();
		$user_tags = ($user_id != 0)? explode(',', get_user_option('tags', $user_id)): array();
		echo ($check_if_user_has_tags) ? 'true' : 'false';
		echo ($check_if_user_has_tags && ! in_array($tag, $user_tags))?'true': 'false' ;
		if ( ! $listing->have_posts() ||
			 ! is_user_logged_in()    ||
			($check_if_user_has_tags && ! in_array($tag, $user_tags) ) ) {
			/**
			 ...
			 */
			return apply_filters( 'slides_shortcode_no_results', wpautop( $no_posts_message ) );
		}
		...
		while ( $listing->have_posts() ): $listing->the_post(); global $post;
			...
			if( $include_author )
				 ...
				 */
				$author = apply_filters( 'slides_shortcode_author', ' <span class="author">by ' . get_the_author() . '</span>', $original_atts );
			...
			if ( $include_excerpt ) {
				...
				if( $excerpt_length || $excerpt_more || $excerpt_more_link ) {
					...
					if( has_excerpt() && apply_filters( 'slides_shortcode_full_manual_excerpt', false ) ) {
			...
			if( $include_content) {
				add_filter( 'shortcode_atts_slides', 'be_slides_off', 10, 3 );
				/** This filter is documented in wp-includes/post-template.php */
				if ($ignore_wrapper)
					$content = apply_filters( 'the_content', get_the_content() );
				else
					$content = '<div class="' . implode( ' ', $content_class ) . '">' . apply_filters( 'the_content', get_the_content() ) . '</div>';
				remove_filter( 'shortcode_atts_slides', 'be_slides_off', 10, 3 );
			}
			...
			if( $category_display && is_object_in_taxonomy( get_post_type(), $category_display ) ) {
				 ...
				 */
				$category_display_text = apply_filters( 'slides_shortcode_category_display', $category_display_text );
			 ...
			 * @param array    $original_atts Original attributes passed to the shortcode.
			 */
			$class = array_map( 'sanitize_html_class', apply_filters( 'slides_shortcode_post_class', $class, $post, $listing, $original_atts ) );
			 ...
			 */
			$inner .= apply_filters( 'slides_shortcode_output', $output, $original_atts, $image, $title, $date, $excerpt, $inner_wrapper, $content, $class );
		 ...
		 * @param array  $original_atts Original attributes passed to the shortcode.
		 */
		$open = ($ignore_wrapper)? '' : apply_filters( 'slides_shortcode_wrapper_open', '<' . $wrapper . $wrapper_class . $wrapper_id . '>', $original_atts );
		 ...
		 * @param array  $original_atts Original attributes passed to the shortcode.
		 */
		$close = ($ignore_wrapper)? '' : apply_filters( 'slides_shortcode_wrapper_close', '</' . $wrapper . '>', $original_atts );
		...
		if( $shortcode_title ) {
			 ...
			 * @param array  $original_atts Original attributes passed to the shortcode.
			 */
			$title_tag = apply_filters( 'slides_shortcode_title_tag', 'h2', $original_atts );

			$return .= '<' . $title_tag . ' class="slides-title">' . $shortcode_title . '</' . $title_tag . '>' . "\n";
		}
...
if ( !function_exists( 'be_sanitize_date_time' ) ) {
	function be_sanitize_date_time( $date_time, $type = 'date', $accepts_string = false ) {
		 ...
		 */
		return apply_filters( 'slides_shortcode_sanitized_segments', $segments, $date_time, $type );
	}
}

/**
 * Turn off display posts shortcode 
 * If display full post content, any uses of [slides] are disabled
 *
...
if ( !function_exists( 'be_slides_off' ) ) {
	function be_slides_off( $out, $pairs, $atts ) {
		/**
		 * Filter whether to disable the slides shortcode.
		 *
		 ...
		 * @since 2.4
		 *
		 * @param bool $disable Whether to disable the slides shortcode. Default true.
		 */
		$out['slides_off'] = apply_filters( 'slides_shortcode_inception_override', true );
		return $out;
	}
}